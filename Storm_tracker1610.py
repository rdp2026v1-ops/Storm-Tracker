...
background_image_path = "C:\Storm_tracker_data/background1610.png"
...
def show_explanation():
    ...
    explanation_window.geometry('780x868+480+5')
    resize_exp_img = Image.open(explanation_path)
    resize_exp_img = resize_exp_img.resize((780, 868), Image.LANCZOS)
...
def play_video(file):
    ...
    video_window.geometry('1200x800+120+40')
...
def define_home():
    ...
    corner3_lat, corner3_lon = 0, 130
    corner4_lat, corner4_lon = 25, 130
    ...
    lat_scale = (screen_height-10) / \
        abs(corner1_lat - corner2_lat)/scale_factor
    lon_scale = (screen_width-10) / abs(corner1_lon - corner4_lon)*scale_factor

    home_lat, home_lon = 7.5786, 108.8694
...
def draw_circle_900():
    ...
    define_home()
    R900_x = screen_width*900/2405
    R900_y = screen_height*900/1495
...
def draw_circle_600():
    ...
    define_home()
    R600_x = screen_width*600/2405
    R600_y = screen_height*600/1495
    circle_600 = cav.create_oval(home_x-R600_x, home_y-R600_y, home_x+R600_x, home_y+R600_y,
                                 outline="yellow", width=4, dash=(2, 2))
...
def draw_circle_300():
    ...
    define_home()
    R300_x = screen_width*300/2405
    R300_y = screen_height*300/1495
...
class StormRotating(object):
    ...
    def draw(self):
        ...
        # image_canvas = tk.Canvas(self.canvas, width=50, height=50)
        # image_canvas.pack()
        # tkimage = ImageTk.PhotoImage(image.rotate(angle))
        # image_canvas.create_image(25, 25, image=tkimage)
...
def track_typhoon():
    ...
    global storm_icon, tracking_counter, lt_text_1, lt_text_2, Tp, distance_to_home
    ...
    try:
        ...
        home_lat, home_lon = 7.578333, 108.869444  # Home point coordinates
        ...
        if lon - home_lon > -1:
            ...
            Tp_label.config(bg='white', text="Typhoon/Storm has passed Lan Tay platform",
                            fg='red', font=("Arial", 14, "bold"), padx=20, pady=20)
        define_home()
        ...
        if wind_speed >= 64:
            ...
            if distance_to_home <= 300:
                warning_label.config(
                    bg="red", text="CAUTIONS - TYPHOON!\nThe Typhoon is within 300NM of the Lan Tay platform - CODE RED\n\nActions to be done: See Checklist for RED zone.", foreground="white", font=("Arial", 12), padx=20, pady=20)
            elif distance_to_home <= 600:
                warning_label.config(
                    bg="yellow", text="CAUTIONS - TYPHOON!\nThe Typhoon is within 600NM of the Lan Tay Platform - CODE YELLOW\n\nActions to be done: See Checklist for YELLOW zone.", foreground="black", font=("Arial", 12), padx=20, pady=20)
            elif distance_to_home <= 900:
                warning_label.config(
                    bg="green", text="CAUTIONS - TYPHOON!\nThe Typhoon is within 900NM of the Lan Tay Platform - CODE GREEN\n\nActions to be done: See Checklist for GREEN zone.", foreground="white", font=("Arial", 12), padx=20, pady=20)
            else:
                warning_label.config(
                    bg="white", text="INFO\nThe Typhoon is not in 900NM of the Lan Tay platform. Be vigilant and stay informed.", font=("Arial", 12))
...
def update_storm_icons():
    ...
    global rel_x, rel_y, rel_ang, storm_conditions, cav, resize_icon_arrow, Locations, location_tuple_2, screen_height, screen_width, lon_scale, lat_scale, corner1_lon, corner1_lat, corner3_lon, corner3_lat
    lat = float(lat_entry.get())
    lon = float(lon_entry.get())
...
def show_checklist_red():
    ...
    checklist_red_window.geometry('980x480+280+10')
...
def show_checklist_yellow():
    ...
    checklist_yellow_window.geometry('980x420+280+10')
...
def show_checklist_green():
    ...
    checklist_green_window.geometry('980x280+280+10')
...
def reset():
    global storm_conditions, Locations, storm_list_box, warning_label, background_image_2, check_var_900, check_var_600, check_var_300, resize_LTP, LTP_image, home_x, home_y
    ...
    if confirmed:
        ...
        # wind_scale_label.config(bg="white")
...
lt_wind_speed_label = tk.Label(
    data_frame, text="LT Wind Speed (kts)", bg='white', fg='black')
...
lt_wind_gust_label = tk.Label(
    data_frame, text="LT Wind Gust (kts)", bg='white', fg='black')
...
track_button = tk.Button(
    data_frame, text="Track Typhoon", command=track_typhoon, width=14, bg='#FFB000')
